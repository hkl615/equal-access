...
Given(/^I am at URL "([^"]*)"$/, async function(url) {
...
When(/^I click on ID "([^"]*)"$/, function(selector) {
    const myThis = this;
    return myThis.driver.waitForSelector(`#${selector}`).then(function() {
        return myThis.driver.click(`#${selector}`);
    });
});
...
Then(/^Page is accessible with label "([^"]*)"$/, { "timeout": 30000 }, async function(label) {
    ...
    try {
        ...
        const buffer = await world.driver.screenshot();
